...
|---|---|---|---|---|
| 1 | **Payload packing + patch-local rows** (24 → 8 B per pair; uint16 angles, uint16 patch-local indices; each patch's rows contiguous → cache-local gathers) | pair memory ÷3; speed: unknown, likely the largest single item | not bit-identical (angle quantisation 2π/65536); extra per-map gather | **in progress** |
| 2 | Tile the fused-3x2pt ξ± section and the density-shear / density-density kernels like `gpu_tiled_tomo_reduce_xipm` | ~4× on those paths | combinations must be compile-time (run-time indexed accumulators spill → 3× slower); ds with custom `ggl_bin_combinations` needs a fallback | only matters for 3x2pt users |
