...
| 11 | Pack pairs already at pair-finding time / in the file (host RAM and disk ÷3) | host memory for k ≳ 5.8 | files no longer exact | **requested as a non-default option, not implemented yet.** Needs: pack at the end of `calculate_pairs_2PCF` (after the treecode ids are shifted, where the row translation is known), a `prepare()` path that uploads packed host arrays directly, and a pair-file flag so `load_pairs` knows the file is quantised |
