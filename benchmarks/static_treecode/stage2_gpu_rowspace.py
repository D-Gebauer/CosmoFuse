"""Stage 2 gate (GPU): compact row space vs the Stage 0 baseline.

Same reference workload as stage0_gpu_baseline.py; additionally feeds the
footprint cut-outs directly as row-space maps and compares every result
bitwise with the Stage 0 results written by the unmodified code.

(Original Stage 0 docstring follows.)
...
def main() -> None:
    ...
    ap.add_argument("--baseline", default=None, help="Stage 0 *_results.npz")
    args = ap.parse_args()
    ...
    t_scatter, t_call_host, t_call_dev, t_call_rows = [], [], [], []
    assert np.array_equal(map_inds, corr.row_pix), 'archive order != row space'
    results = []
    ...
    for k in range(nsets):
        ...
        # row-space input: the footprint cut-out is already in row order
        rows_in = np.ascontiguousarray(cut[k], dtype=dt)
        sync(corr)
        t0 = time.perf_counter()
        out_rows = corr.get_full_tomo_shear(rows_in, w_host, flip_g1=True, return_device=False)
        sync(corr)
        t_call_rows.append(time.perf_counter() - t0)
        for a, b in zip(out, out_rows):
            assert np.array_equal(np.asarray(a), np.asarray(b)), "row-space != full-sky input"
    ...
    summary = {
        ...
        "call_rowspace_host_inputs_ms": med(t_call_rows),
        "n_active": int(corr.n_active),
    ...
    if args.baseline:
        ...
        for name, idx in (("M_a", 0), ("xip", 1), ("xim", 2)):
            new = np.array([r[idx] for r in results])
            ref = base[name][: new.shape[0]]
            summary[f"bitwise_equal_{name}"] = bool(np.array_equal(new, ref))
            summary[f"maxabsdiff_{name}"] = float(np.max(np.abs(new - ref)))
