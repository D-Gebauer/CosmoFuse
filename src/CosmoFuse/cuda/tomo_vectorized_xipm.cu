 ...
 * pair once and accumulates every tomographic combination -- see
 * pair_tiles.cuh for the tile, the estimator (both orientations of a cross
 * combination summed) and the pair loaders.  The per-(bin, row) kernel above
 * re-streams the geometry 2*ncomb times and remains as the fallback for
 * combination layouts / bin counts the tiles do not cover.
 *
 * Output: out_num [xi+ rows k | xi- rows NCOMB + k] x nbins_total,
 *         out_den [k] x nbins_total, k = row-major upper-triangle index.
 *
 * Grid layout:
 *   blockIdx.x  = angular separation bin index (0 .. nbins_total-1)
 *   threadIdx.x = pair index within the bin (strided loop)
...
{
    const long long bin_flat = (long long)blockIdx.x;
    if (bin_flat >= nbins_total) {   /* uniform per block */
        return;
    }
    const UnpackedPairs<T, C, I> pairs = {ind_i, ind_j, rot_i, rot_j};
    tile_xipm<T, TOMO_BINS_T, ACC>(
        pairs, shear, weights, bin_offsets[bin_flat], bin_offsets[bin_flat + 1],
        out_num, out_den, nbins_total, bin_flat);
}


/* Same on packed pairs (8 B per pair, see pair_tiles.cuh); the map rows are
   the per-(patch, level) gathered blocks, row_base[bin] their first row. */
    ...
    const long long nbins_total)
{
    ...
    tile_xipm<T, TOMO_BINS_T, ACC>(
        packed, shear, weights, bin_offsets[bin_flat], bin_offsets[bin_flat + 1],
        out_num, out_den, nbins_total, bin_flat);
}
