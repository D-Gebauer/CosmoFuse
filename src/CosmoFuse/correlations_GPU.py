...
try:
    ...
    logger.error("Cupy not installed. Please install Cupy or use Correlation_GPU.")
    exit(1)
...
class Correlation_GPU(Correlation):
    ...
    def load_maps(
        ...
        if sumofweights is None:
            self.sumofweights = cp.add.reduceat(
                self.w1[self.inds_gpu[0]] * self.w2[self.inds_gpu[1]],
                self.tot_bins_gpu[:-1],
            )
        else:
    ...
    def xipm(self, g11, g21, g12, g22, w1, w2, sumofweights):
        g2 = (
            w1[self.inds_gpu[0]]
            * ((g11[self.inds_gpu[0]]) + 1j * g21[self.inds_gpu[0]])
            * self.exp2phi_gpu[0]
        )
        g1 = (
            w2[self.inds_gpu[1]]
            * ((g12[self.inds_gpu[1]]) + 1j * g22[self.inds_gpu[1]])
            * self.exp2phi_gpu[1]
        )

        xip = (
            (cp.add.reduceat(g1 * cp.conjugate(g2), self.tot_bins_gpu[:-1]))
            / sumofweights
        ).reshape((self.n_patches, self.nbins))
        xim = (
            (cp.add.reduceat(g1 * g2, self.tot_bins_gpu[:-1])) / sumofweights
        ).reshape((self.n_patches, self.nbins))
    ...
    def __xipm(self):
        g2 = (
            self.w1[self.inds_gpu[0]]
            * ((self.g11[self.inds_gpu[0]]) + 1j * self.g21[self.inds_gpu[0]])
            * self.exp2phi_gpu[0]
        )
        g1 = (
            self.w2[self.inds_gpu[1]]
            * ((self.g12[self.inds_gpu[1]]) + 1j * self.g22[self.inds_gpu[1]])
            * self.exp2phi_gpu[1]
        )
        ...
        xim = (
            (cp.add.reduceat(g1 * g2, self.tot_bins_gpu[:-1])) / self.sumofweights
        ).reshape((self.n_patches, self.nbins))
            ...
            for j in range(i, nzbins):
                if i == j:
                    xip1[k], xim1[k] = self.xipm(
                        g1 * shear_maps_gpu[i, 0],
                        g2 * shear_maps_gpu[i, 1],
                        g1 * shear_maps_gpu[j, 0],
                        g2 * shear_maps_gpu[j, 1],
                        w_gpu[i],
                        w_gpu[j],
                        sumofweights_gpu[0, k],
                    )
...
class Correlation_GPU_lowmem(Correlation_GPU):
    ...
    def xipm(self, g11, g21, g12, g22, w1, w2, sumofweights):
        ...
        xip, xim = cp.zeros((self.n_patches * self.nbins)), cp.zeros(
        ...
        for tot_ind in range(self.n_patches * self.nbins):
            xip[tot_ind] = (
                cp.sum(
                    g1[self.tot_bins_gpu[tot_ind] : self.tot_bins_gpu[tot_ind + 1]]
                    * cp.conjugate(
                        g2[self.tot_bins_gpu[tot_ind] : self.tot_bins_gpu[tot_ind + 1]]
                    )
                )
                / (sumofweights[tot_ind]).real
            )
            xim[tot_ind] = (
                cp.sum(
                    g1[self.tot_bins_gpu[tot_ind] : self.tot_bins_gpu[tot_ind + 1]]
